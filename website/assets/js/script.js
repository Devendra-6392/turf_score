...
(function ($) {
	...
	// Offer slide
	if ($('.events-slide-two').length > 0) {
		$('.events-slide-two').slick({
			slidesToShow: 3,
			slidesToScroll: 1,
			dots: false,
			arrows: false,
			...
			responsive: [
				...
				{ breakpoint: 576, settings: { slidesToShow: 1, slidesToScroll: 1, variableWidth: false, centerMode: false } }
			]
		});
	}
	...
	// Header Fixed
	if ($('.header-two').length > 0) {
		$(window).scroll(function () {
			var scroll = $(window).scrollTop();
			if (scroll > 35) {
				$(".header-two").addClass('fixed');
			}

			else {
				$(".header-two").removeClass('fixed');
			}
		})
	}
	...
	const banner = document.querySelector(".banner-section-two");
	...
	banner.addEventListener("mouseleave", () => {
		bannerImg.style.transform = `translate(0px, 0px)`;
	});
